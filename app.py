...
st.markdown("""
    ...
    /* 검색 결과 행 디자인 */
    .result-row { display: flex; align-items: center; padding: 5px 0; border-bottom: 1px solid #eee; }
    
    /* 모바일 가로 유지 */
    [data-testid="column"] { min-width: 0px !important; flex: 1 !important; }
    div[data-testid="stHorizontalBlock"] { gap: 0.5rem !important; flex-wrap: nowrap !important; }

    /* 금액과 버튼 사이 간격 확보 */
    .price-text { color: red; font-weight: bold; text-align: right; padding-right: 15px; margin: 0; }
...
if 'last_idx' not in st.session_state: st.session_state['last_idx'] = 0
...
for i in range(6):
    with calc_cols[i % 2]:
        ...
        # 이름 입력창
        n_in = st.text_input(f"n_{i}", value=st.session_state['names'][i], key=f"name_{i}", label_visibility="collapsed")
        # 금액 입력창
        p_in = st.number_input(f"p_{i}", value=st.session_state['prices'][i], step=1000, key=f"price_{i}", label_visibility="collapsed")
        
        # 중요: 입력창에 값이 바뀌거나 클릭되면 해당 인덱스를 last_idx로 저장
        if n_in != st.session_state['names'][i] or p_in != st.session_state['prices'][i]:
            st.session_state['names'][i] = n_in
            st.session_state['prices'][i] = p_in
            st.session_state['last_idx'] = i
            
        total_val += p_in

st.markdown(f"### 💰 최종 합계: <span style='color:red;'>{total_val:,}원</span>", unsafe_allow_html=True)
...
out_df = pd.DataFrame({"항목": labels, "내용": st.session_state['names'], "단가": st.session_state['prices']})
b2.download_button("💾 견적 저장", data=out_df.to_csv(index=False).encode('utf-8-sig'), file_name="dodo_price.csv", use_container_width=True)
